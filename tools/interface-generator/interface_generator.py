...
def make_ref_type(interface):
    ...
    fullname = join_name('odrive', name)
...
def regularize_attribute(parent, name, elem, c_is_class):
    ...
    if isinstance(elem['type'], str) and elem['type'].startswith('readonly '):
        elem['typeargs']['odrive.Property.mode'] = 'readonly'
        elem['typeargs']['odrive.Property.type'] = elem['type'][len('readonly '):]
        elem['type'] = InterfaceRefElement(parent.fullname, None, 'odrive.Property', elem['typeargs'])
        if elem['typeargs']['odrive.Property.mode'] == 'readonly' and 'c_setter' in elem: elem.pop('c_setter')
    elif ('flags' in elem['type']) or ('values' in elem['type']):
        elem['typeargs']['odrive.Property.mode'] = elem['typeargs'].get('odrive.Property.mode', None) or 'readwrite'
        elem['typeargs']['odrive.Property.type'] = regularize_valuetype(parent.fullname, to_pascal_case(name), elem['type'])
        elem['type'] = InterfaceRefElement(parent.fullname, None, 'odrive.Property', elem['typeargs'])
        if elem['typeargs']['odrive.Property.mode'] == 'readonly' and 'c_setter' in elem: elem.pop('c_setter')
    else:
...
class InterfaceRefElement():
    ...
    def resolve(self):
        ...
        matching value type exists. If so, the interface type odrive.Property<value_type>
        is returned.
        """
        ...
        if 'odrive.Property.type' in typeargs:
            typeargs['odrive.Property.type'] = resolve_valuetype(self._scope, typeargs['odrive.Property.type'])
        ...
        for probe_scope in [join_name(*scope[:(len(scope)-i)]) for i in range(len(scope)+1)]:
            ...
            if probe_name in interfaces:
                ...
                typeargs['odrive.Property.type'] = value_types[probe_name]
    ...
    def __init__(self, name, fullname, mode, value_type):
        ...
        if mode != 'readonly':
            self.functions['exchange'] = {
                ...
                #'implementation': 'property_exchange<' + value_type['c_name'] + '>'
            }
        else:
            self.functions['read'] = {
                'name': 'read',
                ...
                'out': OrderedDict([('value', {'name': 'value', 'type': value_type})]),
                #'implementation': 'property_read<' + value_type['c_name'] + '>'
            }

        interfaces[fullname] = self # TODO: not good to write to a global here

def make_property_type(typeargs):
    value_type = resolve_valuetype('', typeargs['odrive.Property.type'])
    mode = typeargs.get('odrive.Property.mode', 'readwrite')
...
generics = {
    'odrive.Property': make_property_type # TODO: improve generic support
}
...
def map_to_legacy_endpoint_type(t):
...
def generate_endpoint_for_property(prop, attr_bindto, idx):
    ...
    endpoint_definition = {
        'name': prop['name'],
        'id': idx,
        'type': map_to_legacy_endpoint_type(prop['type'].value_type),
...
def generate_endpoint_table(intf, bindto, idx):
    """
    Generates the legacy endpoint table for a given interface.
    ...
    for k, prop in intf.get_all_attributes().items():
        property_value_type = re.findall(r'^odrive\.Property<([^>]*), (readonly|readwrite)>$', prop['type'].fullname)
    ...
    for k, func in intf.get_all_functions().items():
        ...
        for i, (k_arg, arg) in enumerate(list(func['in'].items())[1:]):
            endpoint, endpoint_definition = generate_endpoint_for_property({
                'name': arg['name'],
                'type': make_property_type({'odrive.Property.type': arg['type'], 'odrive.Property.mode': 'readwrite'})
        ...
        for i, (k_arg, arg) in enumerate(func['out'].items()):
            endpoint, endpoint_definition = generate_endpoint_for_property({
                'name': arg['name'],
                'type': make_property_type({'odrive.Property.type': arg['type'], 'odrive.Property.mode': 'readonly'})
...
for k, item in list(interfaces.items()):
    ...
    if len(k) == 1:
        ...
        if k[:-1] != ['odrive']: # TODO: remove special handling
